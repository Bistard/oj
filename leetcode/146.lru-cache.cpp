...
private:
    int _cap;
    umap<int, pair<int, list<int>::iterator>> _map;
    list<int> _list; // list of keys
private:
    void markAsLatestUsed(umap<int, pair<int, list<int>::iterator>>::iterator it) {
        int key = it->first;
...
public:
    ...
    int get(int key) {
        ...
        // mark the key as the latest usaged. Another solution: _list.splice(_list.begin(), _list, it->second);
        markAsLatestUsed(it);
    ...
    void put(int key, int value) {
        ...
        // found
        if (it != _map.end()) {
            // existed key and we simply need to mark it
            markAsLatestUsed(it);
        } 
        // not found
        else {
            // new key and it exceeds the capacity, we delete the last used key.
            if (_map.size() == _cap) {
