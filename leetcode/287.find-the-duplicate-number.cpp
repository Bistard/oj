...
public:
    int findDuplicate(vector<int>& nums) {
        ...
        do {
            p1 = nums[p1];       // slow ptr goes 1 step
            p2 = nums[nums[p2]]; // fast ptr goes 2 steps
        } while (p1 != p2);
...
// tag: binary search; time: O(nlogn + logn); sapce: O(1)
class BinarySearch {
public:
    int findDuplicate(vector<int>& nums) {
        const int n = nums.size();
...
// tag: sorting: time: O(nlogn + n); sapce: O(1)
class Sorting {
public:
    int findDuplicate(vector<int>& nums) {
        const int n = nums.size();
