    ...
    int maxDistance(vector<vector<int>>& grid) {
        ...
        while (!q.empty()) {
            ...
            FOREACH(dir, directions) {
                int newx = x + dir[0];
                int newy = y + dir[1];
                if (!INL(newx, 0, n) || !INL(newy, 0, m) || grid[newx][newy] != 0) {
                    continue;
                }
...
public:
    int maxDistance(vector<vector<int>>& grid) {
        ...
        REPEAT(x, n) {
            REPEAT(y, m) {
                if (grid[x][y] == 1) {
                    FOREACH(dir, directions) {
                        markWaterDistance(dismap, grid, x + dir[0], y + dir[1], 1);
                    }
                }
            }
        }
    ...
    void markWaterDistance(vvi &dismap, vvi &grid, int x, int y, int dis) {
        if (!INL(x, 0, n) || !INL(y, 0, m)) {
            return;
        }
        ...
        if (dismap[x][y] != -1 && dis >= dismap[x][y]) {
            return;
        }
        ...
        FOREACH(dir, directions) {
            markWaterDistance(dismap, grid, x + dir[0], y + dir[1], dis + 1);
        }
    }
};
// @lc code=end
