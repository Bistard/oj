...
public:
    vector<vector<int>> combine(int n, int k) {
        ...
        while (state >= 0) {
            ...
            // always increment on the current number to avoid repetitions.
            ++sample[state];
            
            /**
             * Current digit exceeds `n`, the state should go backward to let 
             * it refresh the current digit.
             */
            if (sample[state] > n) {
                --state;
            } 
            ...
            /**
             * If the current digit does not exceeds `n` and it is also the last
             * digit of the combination, we save the result.
             */
            else if (state == k - 1) {
            ...
            /**
             * If any of the above situation fails, it means the state machine
             * should copy the previous digit to the current ones until it 
             * reaches the end of the combination.
             */
            else {
                ++state;
