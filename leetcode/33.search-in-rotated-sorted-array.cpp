    ...
    int search(vector<int>& nums, int target) {
        ...
        while (l + 1 != r) {
            ...
            if (!(target < nums[0] == nums[m] < nums[0])) {
                /**
                 * Current number and the target are not in the same partition.
                 * If target is less than nums[0], there are two possibilities:
                 *      1. The array is not rotated and the target is less than
                 *         the minimum.
                 *      2. The array is rotated and the target is on the smaller
                 *         partition.
                 * On either cases, it is fine to keep searching on the right 
                 * side.
                 */
                num = (target < nums[0]) ? -INFINITY : INFINITY;
            }
