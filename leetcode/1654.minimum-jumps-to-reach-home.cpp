...
public:
    int minimumJumps(vector<int>& forbidden, int a, int b, int x) {
        ...
        set<int> fobi;
        FOREACH(num, forbidden) {
            fobi.insert(num);
        }

        // pair<int, bool>: {current position, if previous jump is backwarded}.
        queue<pair<int, bool>> q;
        q.push({0, false});
        vvi visited(2);
        visited[0].resize(6000);
        visited[1].resize(6000);
        visited[0][0] = 1;
		visited[1][0] = 1;
        ...
        while (!q.empty()) {
            ...
            int forward = num + a;
            if (forward < 6000 && !HAS(fobi, forward) && !visited[0][forward]) {
                q.push({forward, false});
                visited[0][forward] = 1;
            }

            int backward = num - b;
            if (backward >= 0 && !HAS(fobi, backward) && !isBackward && !visited[1][backward]) {
