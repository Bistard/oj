...
public:
    int rob(vector<int>& nums) {
        ...
        return max(
            robOriginal(begin + 1, end), 
            robOriginal(begin, end - 1)
        );
    }

    int robOriginal(vi::iterator begin, vi::iterator end) {

        int prev = 0, curr = 0, res = 0;

        while (begin != end) {
            res = max(prev + *begin, curr);
            prev = curr;
            curr = res;
            ++begin;
        }
...
public:
    int rob(vector<int>& nums) {
        ...
        int maxProfit = max(max(dp1[n - 2], dp2[n - 3] + nums[n - 1]), dp2[n - 2]);
