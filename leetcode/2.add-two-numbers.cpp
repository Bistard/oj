...
// tag: runtime error (out of range)
class ConvertToInteger {
public:
    ListNode* addTwoNumbers(ListNode* l1, ListNode* l2) {
        long long num = reverseListAddNumber(l1, 0) + reverseListAddNumber(l2, 0);
    ...
    long long reverseListAddNumber(ListNode *node, long long depth) {
        
        if (node->next) {
            long long num = reverseListAddNumber(node->next, depth + 1);
            return num + (node->val * (pow(10, depth)));
        }
        
        return node->val * (pow(10, depth));
    }
};
// @lc code=end
