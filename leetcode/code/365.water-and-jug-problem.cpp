...
// tag: BFS (brute force) (iteration) (TLE);
class Solution {
public:
    bool canMeasureWater(int jug1, int jug2, int target) {
        if (jug1 + jug2 < target) return false;
        if (jug1 == target) return true;
        if (jug2 == target) return true;
        if (jug1 + jug2 == target) return true;

        set<pii> visited; // FIX: TLE (log(n))
        queue<pii> q;
        q.push({0, 0});

        set<pii> nexts; // FIX: TLE (log(n))
        while (!q.empty()) {
            ...
            nexts.insert({jug1, y}); // fill jug1
            nexts.insert({x, jug2}); // fill jug2
            nexts.insert({0, y});    // pour jug1
            nexts.insert({x, 0});    // pour jug2

            int howMuchToPour = min(jug1 - x, y);
            nexts.insert({x + howMuchToPour, y - howMuchToPour}); // pour jug2 into jug1
            
            howMuchToPour = min(jug2 - y, x);
            nexts.insert({x - howMuchToPour, y + howMuchToPour}); // pour jug1 into jug2
...
// tag: math (GCD) (Bézout's identity);
