...
public:
    int shortestBridge(vector<vector<int>>& grid) {
        ...
        queue<pii> q;
        arr2db(visited, n, n, false);

        // find the edge of the first island
        FOR(i, 0, n) {
            FOR(j, 0, n) {
                if (grid[i][j] == 1) {
                    dfsFirstIsland(grid, visited, q, i, j);
                    goto BFS;
                }
            }
        }
    ...
    void dfsFirstIsland(vvi &grid, vvb &visited, queue<pii> &q, int x, int y) {
        const int n = grid.size();
        ...
        while (!s.empty()) {
            ...
            FOREACH(d, directions) {
                ...
                if (grid[newx][newy] == 1) {
                    ...
                    FOREACH(dd, directions) {
                        if (!INL(newx + dd[0], 0, n) || !INL(newy + dd[1], 0, n)) {
                            continue;
                        }

                        if (grid[newx + dd[0]][newy + dd[1]] == 0) {
                            q.push({newx, newy});
                            break;
                        }
                    }
