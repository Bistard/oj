...
public:
    vector<vector<int>> allPathsSourceTarget(vector<vector<int>>& graph) {
        vvi result;
        vi path { 0 };
        dfsFindAllPaths(result, graph, path, 0, graph.size());
        return result;
    }

    void dfsFindAllPaths(vvi &result, vvi &graph, vi &path, int node, int n) {
        ...
        FOREACH(child, graph[node]) {
            ...
            dfsFindAllPaths(result, graph, path, child, n);
            path.pop_back();
        }
    }
};
