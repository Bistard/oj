...
public:
    int longestSubarray(vector<int>& nums) {
        ...
        while (head < len) {
            // if current is one, we don't need to flip anything, keep track it.
            if (nums[head] == 1) {
                currOnes++;
                head++;
            ...
            // if current is zero, flip it when possible.
            if (currZeros < k) {
                currZeros++;
                currOnes++;
                head++;
            ...
            /**
             * If current is zero, but we can no longer flip, we delete from our
             * tail until reaching a zero.
             */
            while (nums[tail] != 0) {
                currOnes--;
                tail++;
            ...
            // Now tail reaches a zero from the tail, we re-flip the tail-zero.
            tail++;
            
            // Flip our head-zero.
            head++;

            // Sicne tail-zero and head-zero cancels each other, no updates to currOnes.
        }
