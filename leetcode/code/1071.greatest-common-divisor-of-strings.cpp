...
public:
    string gcdOfStrings(string str1, string str2) {
        ...
        const int maxLen = max(str1Len, str2Len);
        const int minLen = min(str1Len, str2Len);
        string &minStr = str1Len > str2Len ? str2 : str1;
        string &maxStr = str1Len > str2Len ? str1 : str2;
        
        int i = 0;
        for (i = 0; i < maxLen; i++) {
            const char maxC = maxStr[i];
            const char minC = minStr[i % minLen];
        ...
        return gcdOfStrings(minStr, minStr.substr(i % minLen));
    }
};
// @lc code=end
