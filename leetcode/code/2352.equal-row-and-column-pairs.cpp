...
public:
    int equalPairs(vector<vector<int>>& grid) {
        ...
        vector<long long> rowHash;
        vector<long long> colHash;
        
        // hash every row and col O(n^2)
        vi arr(n, 0);
        REPEAT(i, n) {    
            // iterate by row
            REPEAT(j, n) {
            ...
            rowHash.push_back(hash(arr));

            // iterate by col
            REPEAT(j, n) {
                arr[j] = grid[j][i];
            }
            colHash.push_back(hash(arr));
        }

        // compare every row to col O(n^2)
        REPEAT(i, n) {
            REPEAT(j, n) {
    ...
    long long hash(vi &arr) {

        long long hash = 0;
        for (int num : arr) {
            hash = (hash * (long long)17 + num) % (long long)MODULO;
        }
        return hash;
    }
};
