...
public:
    int compress(vector<char>& chars) {
        ...
        FOR(i, 1, oldLen) {
            ...
            newLen = pushCompressed(chars, newLen, curr, cnt);
            cnt = 1;
            curr = chars[i];
        }
        ...
        // remove the uncompressed chars
        chars.erase(chars.begin(), chars.begin() + oldLen);

        return newLen;
    }

    int pushCompressed(vector<char>& chars, int newLen, char curr, int cnt) {
        chars.push_back(curr);
        newLen++;
        
        if (cnt != 1) {
            int length = log10(cnt) + 1;
            for (int i = length - 1; i >= 0; i--) {
                int divisor = pow(10, i);
                int digit = cnt / divisor;
                cnt %= divisor;

                chars.push_back(digit + '0');
                newLen++;
            }
        }
