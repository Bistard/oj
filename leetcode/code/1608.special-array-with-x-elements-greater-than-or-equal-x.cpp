...
public:
    int specialArray(vector<int>& nums) {
        ...
        // binary search to find the special number
        int l = -1, r = nums[n - 1] + 1;
        int m;
        while (l + 1 != r) {
            m = l + (r - l) / 2;
            int greaterCnt = n - binarySearchToFindTheIndexOfTheFirstGreaterNumber(nums, m);

            // the special number is in the array
            if (m == greaterCnt) {
                return m;
            }
    ...
    int binarySearchToFindTheIndexOfTheFirstGreaterNumber(vi &arr, int target) {
        int l = -1, r = arr.size();
        int m;
        while (l + 1 != r) {
            m = l + (r - l) / 2;
            ...
            if (arr[m] == target) {
                int idx = m;
                while (idx - 1 >= 0 && arr[idx - 1] == target) {
                    --idx;
                }
                return idx;
            }
