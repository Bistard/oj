...
public:
    bool canPlaceFlowers(vector<int>& flowerbed, int n) {
        ...
        // no ones
        if (oneIdx.size() == 0) {
            return (n == 1 && len == 1) || (len / 2 + len % 2) >= n;
        }

        // left and right
        int directions[2] = {-2, 2};


        while (oneIdx.size() > 0) {
            const int idx = oneIdx.top();
            oneIdx.pop();
            ...
            for (int dir : directions) {
                ...
                // empty spot
                if (flowerbed[newIdx] != 1) {
                    if (nextToOne(flowerbed, newIdx, len)) {
                        continue;
                    }
    ...
    bool nextToOne(vi &flowered, int idx, int len) {
        if (idx - 1 >= 0 && flowered[idx - 1] == 1) {
            return true;
        }

        if ((idx + 1 <= len - 1) && flowered[idx + 1] == 1) {
            return true;
        }
