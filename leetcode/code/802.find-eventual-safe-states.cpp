...
public:
    vector<int> eventualSafeNodes(vector<vector<int>>& graph) {
        ...
        // 0: not visited, 1: terminal, 2: safe.
        vi visited(n, 0);
        vi safe;

        // find all the safe nodes using DFS.
        REPEAT(node, n) {
            if (dfs(graph, visited, safe, node)) {
                safe.push_back(node);
            }
        }
    ...
    bool dfs(vvi &graph, vi &visited, vi &safe, int node) {

        if (visited[node] == SAFE) {
            return true;
        } else if (visited[node] == NOTSAFE) {
            return false;
        }

        visited[node] = NOTSAFE;

        bool isSafe = true;
        FOREACH(child, graph[node]) {
            isSafe = dfs(graph, visited, safe, child) && isSafe;
        }
