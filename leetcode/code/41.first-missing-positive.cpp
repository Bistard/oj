...
public:
    int firstMissingPositive(vector<int>& nums) {
        ...
        /**
         * The main idea is to keep every nums[i] = i + 1. Once the process is
         * done, we loop it again to see if any nums[i] != i and that (i + 1)
         * will be the smallest positive integer that is missing.
         */
        REPEAT(i, n) {
            /**
             * 
             * If the current number num[i] is in the range [0, n] and the 
             * number in its corresponding index nums[nums[i] - 1] is not placed
             * with the correct value, we swap it to make sure at least 
             * nums[nums[i] - 1] is placed with the correct value.
             */
            while (INR(nums[i], 0, n) && nums[i] != nums[nums[i] - 1]) {
...
// tag: brute force search (binary search); time: O(nlogn);
class BinarySearch {
public:
    int firstMissingPositive(vector<int>& nums) {
        SORT(nums);
        ...
        int num = 1;
        while (binarySearch(nums, num) == true) {
            num++;
        }
    ...
    bool binarySearch(vi &nums, int num) {
        int l = -1, r = nums.size();
        int m;
