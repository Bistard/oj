...
public:
    vector<vector<string>> accountsMerge(vector<vector<string>>& accounts) {
        ...
        vi parents(n);
        umap<string, int> graph; // for each mail, returns who has the mail.
        ...
        FOREACH(p, graph) {
            ...
            const int root = find(parents, parent);
            if (!HAS(persons, root)) {
                persons[root] = vs();
            }
            persons[root].push_back(mail);
        }
        ...
        FOREACH(p, persons) {
            ...
            auto &mails = p.second;
            SORT(mails);
            mails.insert(mails.begin(), accounts[idx][0]);
            results.emplace_back(mails);
        }
