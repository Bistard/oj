...
// tag: DFS (recursion)
class MyStupidRecursionVersion {
private:
    bool isValid = true;
public:
    bool isValidBST(TreeNode* root) {
        ...
        if (root->left) {
            auto [leftMax, leftMin] = dfs(root->left);
        ...
        if (root->right) {
            auto [rightMax, rightMin] = dfs(root->right);
            if (rightMax <= root->val || rightMin <= root->val) {
                return false;
            }
        }
    ...
    pair<int, int> dfs(TreeNode *node) {
        ...
        if (node->left) {
            auto [leftMax, leftMin] = dfs(node->left);
            
            if (leftMax >= node->val || leftMin >= node->val) {
                isValid = false;
            }
        ...
        if (node->right) {
            auto [rightMax, rightMin] = dfs(node->right);

            if (rightMin <= node->val || rightMax <= node->val) {
                isValid = false;
            }
