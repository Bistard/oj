...
public:
    int countSubIslands(vector<vector<int>>& grid1, vector<vector<int>>& grid2) {
        ...
        REPEAT(i, n) {
            REPEAT(j, m) {
                if (grid2[i][j] == 1) {
                    bool isSubIland = dfs(grid2, grid1, i, j);
                    if (isSubIland) {
                        ++cnt;
                    }
                }
    ...
    bool dfs(vvi &grid, vvi &otherGird, int x, int y) {
        if (INL(x, 0, n) && INL(y, 0, m) && 
            grid[x][y] == 1
        ) {
            grid[x][y] = 2;
            bool isSub = (otherGird[x][y] == 1);
            FOREACH(dir, directions) {
                isSub = dfs(grid, otherGird, x + dir[0], y + dir[1]) && isSub;
            }
