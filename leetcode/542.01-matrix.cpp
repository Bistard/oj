...
public:
    vector<vector<int>> updateMatrix(vector<vector<int>>& mat) {
        ...
        // find all the adjcent 1's that next to 0's.
        REPEAT(i, n) {
            REPEAT(j, m) {
                if (mat[i][j] == 1) {
                    FOREACH(d, directions) {
                        int newx = i + d[0], newy = j + d[1];
                        if (INL(newx, 0, n) && INL(newy, 0, m) && mat[newx][newy] == 0) {
                            q.push({i, j});
                            visited[i][j] = true;
                            break;
                        }
        ...
        while (!q.empty()) {
            ...
            FOREACH(d, directions) {
                ...
                if (INL(newx, 0, n) && INL(newy, 0, m) && mat[newx][newy] == 1 && !visited[newx][newy]) {
