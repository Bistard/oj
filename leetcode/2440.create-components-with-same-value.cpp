...
class Solution {
public:
    int componentValue(vi &nums, vvi &edges) {
        
        // build graph into 2d map
        vvi graph = buildGraph(nums, edges);

        // get the total sum of the whole graph
        const int sum = SUM(nums);

        // iterate each possible divisor that can be treated as the number of components.
        for (int i = sum; i >= 2; --i) {
            int compCnt = i;
            int compVal = sum / compCnt;
            ...
            dfs(graph, nums, 0, -1, compCnt, compVal);
            
            /**
             * As long as the current `compCnt` successed, we immediately return
             * it since we already found the largest components that can be 
             * formed because we are looping from backwards.
             */
            if (compCnt == 0) {
                return i - 1;
            }
    ...
    vvi buildGraph(vi &nums, vvi &edges) {
    ...
    int dfs(vvi &graph, vi &nums, int node, int parent, int &compCnt, int compVal) {
        int totVal = nums[node];

        FOREACH(child, graph[node]) {
            if (child != parent) {
                totVal += dfs(graph, nums, child, node, compCnt, compVal);
            }
        }

        /**
         * The current node and its children can form a component with the same
         * target value.
         */
        if (totVal == compVal) {
            --compCnt;
        ...
        /**
         * If not, we simply return the current values back to the parent to see
         * if they can form a component.
         */
        return totVal;
    }
};
