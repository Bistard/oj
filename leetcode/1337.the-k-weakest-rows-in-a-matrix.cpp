...
public:
    vector<int> kWeakestRows(vector<vector<int>>& mat, int k) {
        ...
        priority_queue<pii, vector<pii>, CompareFn> minMap;

        REPEAT(i, n) {
            int numOfOne = binarySearchOnRow(mat[i]);
            minMap.push({numOfOne, i});
        }

        vi result;
        REPEAT(i, k) {
            result.push_back(minMap.top().second);
            minMap.pop();
        }
