...
// tag: DP (bottom-up), space - O(h)
class Solution {
public:
    int minimumTotal(vector<vector<int>>& triangle) {
        int n = triangle.size();
        vi minlen(triangle.back());
...
public:
    int minimumTotal(vector<vector<int>>& triangle) {
        ...
        FOREACH(row, triangle) VALUE(row);

        auto it = min_element(begin(triangle[h - 1]), end(triangle[h - 1]));
        return *it;
    }
};
// @lc code=end
