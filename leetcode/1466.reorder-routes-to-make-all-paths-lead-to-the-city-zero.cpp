...
public:
    int minReorder(int n, vector<vector<int>>& connections) {
        ...
        /**
         * Traversing using DFS of all the nodes starting from capital (0). It
         * is guarranteed that all the nodes can be visited since it is a tree.
         * We simply count how many edges that are out-going from capital (0)
         * and return it.
         */

        vvi graph(n, vi {});
        vb visited(n, false);
        FOREACH(e, connections) {
    ...
    int dfs(vvi &graph, vb &visited, int parent) {
        ...
        FOREACH(child, graph[parent]) {
            if (visited[abs(child)]) continue;
            
            if (child > 0) {
                distance++;
            }

            distance += dfs(graph, visited, abs(child));
        }
