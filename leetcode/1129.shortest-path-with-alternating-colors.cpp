...
public:
    vi shortestAlternatingPaths(int n, vvi &redEdges, vvi &blueEdges) {
        ...
        vvi redG = buildGraph(n, redEdges);
        vvi blueG = buildGraph(n, blueEdges);
        vb redV(n, false);
        vb blueV(n, false);
        ...
        while (!q.empty()) {
            ...
            if (color == RED) {
                redV[node] = true;
                FOREACH(child, blueG[node]) {
                    if (blueV[child]) continue;
                    q.push({child, BLUE});
                    if (result[child] == -1) result[child] = distance;
                    else SETMIN(result[child], distance);
                }
            }
            else {
                blueV[node] = true;
                FOREACH(child, redG[node]) {
                    if (redV[child]) continue;
                    q.push({child, RED});
                    if (result[child] == -1) result[child] = distance;
                    else SETMIN(result[child], distance);
                }
    ...
    vvi buildGraph(int n, vvi &edges) {
        vvi graph(n, vi {});
