...
// time: O(n);
class IfIAlreadyLoopTheArrayToFindThePivotWhyCouldntIJustFindTheNumInstead {
public:
    bool search(vector<int>& nums, int target) {
        ...
        // rset pivot if needed
        if (pivot == 0) pivot = n;
        
        // binary search
        int l = target < nums[0] ? (pivot - 1) : -1;
        int r = target < nums[0] ? n : pivot;
        int m;
        while (l + 1 != r) {
            m = l + (r - l) / 2;
            if (nums[m] == target) {
